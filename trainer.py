...
try:
    ...
    logging.warning("You should install visdom if you want to use it!")
...
class Trainer(TrainerBase):
    def __init__(self,
                 ...
                 visdom_object:Optional[vis.Visdom]=None
                 ) -> None:
        """
        ...
        #########################################################
        self.visdom = visdom_object
        self.train_loss_window=None
        self.i=1
        #########################################################
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_group in train_generator_tqdm:
            ...
            if self.train_loss_window:
                self.visdom.line(X=np.array([self.i]), Y=np.array([loss.item()]),
                 win=self.train_loss_window, update='append')
                self.i+=1
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self.visdom:
            ...
            self.train_loss_window = create_plot_window(self.visdom, '#Iterations', 'Loss', 'Training Loss')
            self.consume_time_window=create_plot_window(self.visdom,"#Epochs","Seconds","Consuming time")
            self.left_time_window=self.visdom.text("Waiting for training.......")
            metric_window = {}
        ##########################################################################################
        ...
        for epoch in range(epoch_counter, self._num_epochs):
            ...
            if self.visdom:
                for key in train_metrics.keys():
                    ...
                    if newkey in metric_window:
                        ...
                        metric_window[newkey] = create_plot_window(self.visdom, '#Epochs', key, newkey)
            ...
            for key, value in train_metrics.items():
                ...
                ##########################################################
                if self.visdom:
                    self.visdom.line(X=np.array([epoch]), Y=np.array([value]),
                                     win=metric_window["training_" + key], update='append')
                #########################################################
            ...
            for key, value in val_metrics.items():
                ...
                if self.visdom:
                    self.visdom.line(X=np.array([epoch]), Y=np.array([value]),
                                     win=metric_window["validation_" + key], update='append')
                ############################################################
            ...
            #######################################################################################
            if self.visdom:
                self.visdom.line(X=np.array([epoch]), Y=np.array([epoch_elapsed_time/60]),
                                 win=self.consume_time_window, update='append')
            ############################################################################################
            if epoch < self._num_epochs - 1:
                ...
                #######################################################################################
                if self.visdom:
                    self.visdom.text("Estimated training time remaining: {}".format(formatted_time),win=self.left_time_window,
                                     append=True)
                ############################################################################################
            epochs_trained += 1
